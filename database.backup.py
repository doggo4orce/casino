...
class database:
  ...
  """create_tables()         <-- creates all database tables
     ...
     contains_exit(rm, dir)  <-- check if the exit will collide with an existing exit
  ...
  def execute(self, line, parameters=()):
    try:
      ...
      mudlog.error(f"SQL Programming Error with line {line} and parameters {parameters}.")
    else:
      self._conn.commit()
      return ret_val
  ...
  def save_exit(self, rm, ex):
    if self.contains_exit(rm, ex.direction):
      self._delete_exit(rm, ex.direction)
  ...
  def load_stock(self):
    ...
    rm.id = "void"
    rm.desc = "<p>This is a nice, calm, relaxing space. Anything in this room probably wound up here because it's last known location no longer exists. Head down to return to recall.</p>"
...
<p>But now I've entered paragraph mode again. So all of this text will be formatted according to my user-set preference of how wide I want my screen to be.</p>"""
    ...
    npcp = npc_proto.npc_proto_data()
    npcp.reset_aliases('baccarat', 'dealer')
    npcp.entity.name = 'the baccarat card dealer'
    npcp.entity.desc = "<p>He looks like he's straight out of a bluegrass music video.</p>"
    ...
    stockville._npc_proto[npcp.id] = npcp

    npcp = structs.npc_proto_data()
    npcp.reset_aliases('baker', 'fat')
    npcp.name = 'the baker'
    npcp.desc = "<p>He's a nice looking person, but you can see that he has seen battle by the many scars on his body.</p>"
    ...
    op.desc = editor.buffer("<p>It's a <c6>black<c0> coin with a Stockville Casino logo imprinted upon it.</p>")
    ...
    op.desc = editor.buffer("<p>It's a <c5>purple<c0> coin with a Stockville Casino logo imprinted upon it.</p>")
    ...
    op.name = 'a bottle'
    op.desc = editor.buffer("<p>It's brown, sticky, and smells like stale beer inside.</p>")
